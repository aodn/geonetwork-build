...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- Identify missing acquisition information platform instruments -->
  ...
  <xsl:variable name="keyword-platform-instrument-combinations">
    <xsl:for-each select="$keyword-platforms">
      <xsl:variable name="platform" select="*/text()"/>
      <xsl:for-each select="$keyword-instruments">
        <xsl:variable name="instrument" select="*/text()"/>
        <platform-instrument><xsl:value-of select="concat($platform,'/',$instrument)"/></platform-instrument>
      </xsl:for-each>
    </xsl:for-each>
  </xsl:variable>

  <xsl:variable name="acquisition-platform-instruments">
    <xsl:for-each select="//mac:MI_Platform/*/mac:MI_Instrument">
      <xsl:variable name="platform" select="ancestor::mac:MI_Platform/mac:identifier/mcc:MD_Identifier/mcc:code/*/text()"/>
      <xsl:variable name="instrument" select="mac:identifier/mcc:MD_Identifier/mcc:code/*/text()"/>
      <platform-instrument><xsl:value-of select="concat($platform,'/',$instrument)"/></platform-instrument>
    </xsl:for-each>
  </xsl:variable>

  <xsl:variable name="missing-acquisition-platform-instruments"
                select="$keyword-platform-instrument-combinations/*[not(index-of($acquisition-platform-instruments/*/text(), .))]/text()"/>
  ...
  <xsl:template name="analyze-add-missing-instruments">
    ...
    <xsl:if test="$missing-coverage-parameters or $missing-acquisition-platforms or $missing-acquisition-platform-instruments">
      <xsl:variable name="missing-params-msg">
        <xsl:if test="$missing-coverage-parameters">
          <xsl:value-of select="concat('Add ''', string-join($missing-coverage-parameters/*/text(),', '), ''' to coverage description.')"/>
        </xsl:if>
      </xsl:variable>
      <xsl:variable name="missing-platforms-msg">
        <xsl:if test="$missing-acquisition-platforms">
          <xsl:value-of select="concat('Add ''', string-join($missing-acquisition-platforms/*/text(),', '), ''' to acquisition information.')"/>
        </xsl:if>
      </xsl:variable>
      <xsl:variable name="missing-platform-instruments-msg">
        <xsl:if test="$missing-acquisition-platform-instruments">
          <xsl:value-of select="concat('Add ''', string-join($missing-acquisition-platform-instruments,', '), ''' to acquisition information.')"/>
        </xsl:if>
      </xsl:variable>
      <suggestion process="add-missing-instruments">
        <name><xsl:value-of select="concat($missing-params-msg, ' ', $missing-platforms-msg, ' ', $missing-platform-instruments-msg)"/></name>
